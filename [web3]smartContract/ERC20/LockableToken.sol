...
contract SimpleLockableToken is ERC20Interface {
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _addressTokenLock;
    ...
    function isTokenLock(address from, address to) public view returns (bool lock) {
        ...
        if(_addressTokenLock[from] == true || _addressTokenLock[to] == true) {
             lock = true;
        }
    }

    function isTokenLock() external view returns (bool lock) {
        lock = false;
        if(_tokenLock == true)
        {
            lock = true;
    ...
    function removeAddressLock(address _who) onlyOwner public {
        require(_addressTokenLock[_who] == true);
        _addressTokenLock[_who] = false;
    }
    ...
    function addAddressTokenLock(address _who) onlyOwner public {
        require(_addressTokenLock[_who] == false);
        _addressTokenLock[_who] = true;
    }
